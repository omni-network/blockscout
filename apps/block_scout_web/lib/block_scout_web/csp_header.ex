  ...
  def call(conn, _opts) do
    ...
    Controller.put_secure_browser_headers(conn, %{
      "content-security-policy" => "\
        connect-src 'self' http://localhost:8888 https://testnet-xapi.explorer.omni.network https://staging-xapi.explorer.omni.network #{json_rpc_url} #{config[:mixpanel_url]} #{config[:amplitude_url]} #{websocket_endpoints(conn)} #{czilladx_url} #{trustwallet_url} #{walletconnect_urls};\
        default-src 'self';\
